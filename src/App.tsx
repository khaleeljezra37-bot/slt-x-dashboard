...
export default function App() {
  ...
  const copyToClipboard = (text: string, type: 'bypass' | 'refresh' | 'checker') => {
    ...
    <div className="flex h-screen bg-[#0a0a0a] text-white font-sans overflow-hidden">
      ...
      >
        ...
        <nav className="flex-1 px-4 space-y-1 overflow-y-auto overflow-x-hidden whitespace-nowrap pb-4">
          <NavItem icon={<LayoutDashboard size={20} />} label="Dashboard" active={activeTab === 'Dashboard'} onClick={() => { setActiveTab('Dashboard'); if (isMobile) setSidebarOpen(false); }} />
          <NavItem icon={<ShieldCheck size={20} />} label="Bypasser" active={activeTab === 'Bypasser'} onClick={() => { setActiveTab('Bypasser'); if (isMobile) setSidebarOpen(false); }} />
          <NavItem icon={<RefreshCw size={20} />} label="Refresher" active={activeTab === 'Refresher'} onClick={() => { setActiveTab('Refresher'); if (isMobile) setSidebarOpen(false); }} />
          <NavItem icon={<UserSearch size={20} />} label="Account Checker" active={activeTab === 'Account Checker'} onClick={() => { setActiveTab('Account Checker'); if (isMobile) setSidebarOpen(false); }} />
          <NavItem icon={<BookOpen size={20} />} label="Tutorials" active={activeTab === 'Tutorials'} onClick={() => { setActiveTab('Tutorials'); if (isMobile) setSidebarOpen(false); }} />

          <div className="my-4 border-t border-[#1f1f1f]"></div>

          <NavItem icon={<Lock size={20} />} label="Admin Panel" active={activeTab === 'Admin Panel'} onClick={() => { setActiveTab('Admin Panel'); if (isMobile) setSidebarOpen(false); }} />
          <NavItem icon={<MessageSquare size={20} />} label="Discord Community" active={activeTab === 'Discord Community'} onClick={() => { window.open('https://discord.gg/qZePjjmb7k', '_blank'); if (isMobile) setSidebarOpen(false); }} />
        </nav>
      ...
      <main className="flex-1 flex flex-col h-screen overflow-hidden relative z-10 w-full">
        ...
        <div className="flex-1 overflow-y-auto p-4 sm:p-6 md:p-8 lg:p-12">
          {activeTab === 'Dashboard' && (
            <>
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4 mb-6 md:mb-8">
                <StatItem icon={<Activity size={20} />} label="Total Bypasses" value={totalBypasses.toString()} />
                <StatItem icon={<UserSearch size={20} />} label="Accounts Checked" value={totalChecks.toString()} />
                <StatItem icon={<RefreshCw size={20} />} label="Cookies Refreshed" value={totalRefreshes.toString()} />
                <StatItem icon={<ShieldCheck size={20} />} label="Success Rate" value="99.9%" />
              </div>
              ...
              <div className="max-w-2xl border border-[#1f1f1f] bg-[#0a0a0a] p-6 md:p-8 rounded-2xl">
                ...
                <div className="space-y-6">
                  <div>
                    ...
                    <p className="text-xs text-gray-500 mt-2">Enter the target account's cookie to initiate the bypass process. <span className="text-gray-400 font-semibold italic">Note: The cookie must contain the standard Roblox warning message.</span></p>
                  </div>
                  ...
                  >
                    {loading ? 'Executing...' : 'Execute Bypass'} <ChevronRight size={16} />
                  </button>
                    ...
                    <div className={`mt-6 p-5 rounded-2xl border flex gap-4 items-start transition-all duration-300 ${
                      ...
                      <div className="flex-1">
                        ...
                        <p className="text-sm text-gray-400 leading-relaxed">
                          {response.result?.message || response.message || (response.error ? response.error : 'An unknown error occurred.')}
                        </p>
                        {response.result?.username && response.result.username !== 'Unknown' && (
                          <p className="text-[10px] mt-2 font-mono uppercase tracking-widest text-gray-500">
                            Target: {response.result.username}
                          </p>
                        )}
                        {response.result?.success && response.result?.content && (
                          <div className="mt-4">
                            ...
                            <div className="relative group">
                              ...
                              />
                              <button 
                                onClick={() => copyToClipboard(response.result.content, 'bypass')}
                              ...
                              >
                                ...
                                {copiedBypass && <span className="text-[10px] font-bold">COPIED</span>}
                              </button>
                            </div>
                          </div>
                        )}
              ...
              <div className="max-w-2xl border border-[#1f1f1f] bg-[#0a0a0a] p-6 md:p-8 rounded-2xl">
                ...
                <div className="space-y-6">
                  <div>
                    ...
                    <p className="text-xs text-gray-500 mt-2">Enter the target account's cookie to refresh it. <span className="text-gray-400 font-semibold italic">Note: The cookie must contain the standard Roblox warning message.</span></p>
                  </div>
                  ...
                  >
                    {refresherLoading ? 'Refreshing...' : 'Execute Refresh'} <ChevronRight size={16} />
                  </button>
                  ...
                  {refresherResponse && (
                    <div className={`mt-6 p-4 border rounded-2xl flex gap-4 ${
                      ...
                      <div className="flex-1">
                        ...
                        <p className="text-sm text-gray-400 leading-relaxed">
                          {refresherResponse.result?.message || refresherResponse.message || (refresherResponse.error ? refresherResponse.error : 'An unknown error occurred.')}
                        </p>
                        {refresherResponse.result?.success && refresherResponse.result?.content && (
                          <div className="mt-4">
                            ...
                            <div className="relative group">
                              ...
                              />
                              <button 
                                onClick={() => copyToClipboard(refresherResponse.result.content, 'refresh')}
                                className="absolute top-2 right-2 p-1.5 bg-green-500/20 hover:bg-green-500/40 text-green-400 rounded-2xl transition-colors flex items-center gap-1"
                                title="Copy to clipboard"
                              >
                                {copiedRefresh ? <CheckCircle2 size={14} /> : <Copy size={14} />}
                                {copiedRefresh && <span className="text-[10px] font-bold">COPIED</span>}
                              </button>
                            </div>
                            <p className="text-[10px] text-gray-500 mt-1 italic">This is your new session cookie. Keep it safe.</p>
                          </div>
                        )}
                      </div>
                    </div>
            ...
            <>
              ...
              <div className="max-w-2xl border border-[#1f1f1f] bg-[#0a0a0a] p-6 md:p-8 rounded-2xl">
                ...
                <div className="space-y-6">
                  ...
                  >
                    {checkerLoading ? 'Scanning...' : 'Execute Scan'} <ChevronRight size={16} />
                  </button>
                      ...
                      {checkerResponse.result?.status === 'success' && checkerResponse.result.user_info && (
                        <div className="mt-4 bg-[#0a0a0a] border border-[#1f1f1f] rounded-2xl overflow-hidden">
                            ...
                            <div>
                              ...
                              {checkerResponse.result.presence?.label && (
                                <div className="flex items-center gap-1.5 mt-1">
                                  ...
                                  <span className="text-xs text-gray-400">{checkerResponse.result.presence.label}</span>
                                </div>
                              )}
                            </div>
                          </div>
                          <div className="p-4 grid grid-cols-2 md:grid-cols-4 gap-4">
                            ...
                            <div>
                              <p className="text-[10px] text-gray-500 uppercase tracking-wider font-bold mb-1">Age Status</p>
                              <p className="text-white font-mono">{checkerResponse.result.user_info.age_status}</p>
                            </div>
                            <div>
                            ...
                            <div>
                              <p className="text-[10px] text-gray-500 uppercase tracking-wider font-bold mb-1">2FA Enabled</p>
                              <p className="text-white font-mono">{checkerResponse.result.account_status?.two_factor_auth}</p>
                            </div>
                            <div>
                              <p className="text-[10px] text-gray-500 uppercase tracking-wider font-bold mb-1">Voice Chat</p>
                              <p className="text-white font-mono">{checkerResponse.result.account_status?.voice_chat}</p>
                            </div>
                            <div>
                              <p className="text-[10px] text-gray-500 uppercase tracking-wider font-bold mb-1">ID Verified</p>
                              <p className="text-white font-mono">{checkerResponse.result.account_status?.age_verified}</p>
                            </div>
                          </div>
                          
                          <div className="p-4 border-t border-[#1f1f1f]">
                            <p className="text-[10px] text-gray-500 uppercase tracking-wider font-bold mb-3">Game Data & Activity</p>
                            <div className="grid grid-cols-1 md:grid-cols-3 gap-3">
                              {Object.entries(checkerResponse.result.games?.per_game || {}).map(([name, data]: [string, any]) => (
                                <div key={name} className="bg-[#141414] p-3 rounded-2xl border border-[#1f1f1f]">
                                  <div className="flex justify-between items-center mb-2">
                                    <p className="text-xs font-bold text-white uppercase">{name.replace(/_/g, ' ')}</p>
                                    <div className={`w-2 h-2 rounded-full ${data.played_proxy ? 'bg-green-500' : 'bg-gray-600'}`}></div>
                                  </div>
                                  <p className="text-[10px] text-gray-400">
                                    {data.played_proxy ? 'Played / Voted' : 'No activity detected'}
                                  </p>
                                </div>
                              ))}
                            </div>
                          ...
                          <div className="p-4 border-t border-[#1f1f1f] bg-[#0d0d0d]">
                            <p className="text-[10px] text-gray-500 uppercase tracking-wider font-bold mb-1">Inventory Note</p>
                            <p className="text-xs text-gray-400 italic">
                              {checkerResponse.result.inventory?.note}
                            </p>
                          </div>
                        </div>
                      )}
          ...
          {activeTab !== 'Dashboard' && activeTab !== 'Bypasser' && activeTab !== 'Refresher' && activeTab !== 'Account Checker' && activeTab !== 'Tutorials' && (
            <div className="flex flex-col items-center justify-center h-64 text-gray-400">
              <p className="text-lg font-medium mb-2">{activeTab} Module</p>
